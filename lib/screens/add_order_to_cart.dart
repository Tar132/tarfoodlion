...
class AddOrderToCart extends StatefulWidget {
  ...
  AddOrderToCart({Key key, this.foodModel}) : super(key: key); //construtor
  @override
...
class _AddOrderToCartState extends State<AddOrderToCart> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('เพิ่มรานการอาหารไป ตะกร้า'),
      ),
      body: Stack(
        children: <Widget>[
          showContent(),
              ...
              Row(
                children: <Widget>[
                  ...
                  IconButton(
                      ...
                      onPressed: () {
                        if (amount == 1) {
                          normalDialog(context, 'ไม่สามารถลดลงได้อีกแล้ว');
                        } else {
                          setState(() {
                            amount--;
                          });
  ...
  Future<Null> checkAndSaveSQLite() async {
    ...
    String idShopFromSQLite = orderSQLiteModels.length == 0? foodModel.idShop:orderSQLiteModels[0].idShop;
    if (idShopFromSQLite == foodModel.idShop) {
      saveOrderToSQLite();
    } else {
      normalDialog(context,
          'กรุณาเลือกรายการอาหารจากร้าน ${orderSQLiteModels[0].nameShop} หรือ Order ไปก่อนคะ');
    }
    saveOrderToSQLite();
  }
