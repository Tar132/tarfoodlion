...
class _ShowCartState extends State<ShowCart> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
          onPressed: () => orderThread(), child: Text('สั่งซื้อ')),
      appBar: AppBar(
      ...
      body: orderSQLiteModels.length == 0
          ? Center(
              child: Text('ยังไม่มีของใน ตะกร้า คะ'),
            )
          : showListFood(),
    );
  }
  ...
  Future<Null> orderThread() async {
    ...
    await Dio().get(url).then(
      (value) async {
        SQLiteHelper().clearSQLite().then((value) {
          backHome(context);
        });
      },
    );
  ...
  Future<Null> confirmDelete(int id, String nameFood) async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('คุณต้องการลบ $nameFood จริงๆหรือ'),
        children: <Widget>[
          Row(
  ...
  Future<Null> delete(int idFood) async {}
}
